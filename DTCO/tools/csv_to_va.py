...
def add_derivatives_to_iv_csv(input_file, output_file=None):
    ...
    print("  开始计算gm和gds...")
    sys.stdout.flush()
    for idx, (device_key, device_df) in enumerate(tqdm(device_groups_list, desc="处理设备组", total=num_groups)):
...
def train_dc_model(csv_path, output_dir, device_type='n', num_epochs=200):
    ...
    for epoch in range(num_epochs):
        ...
        model.train()
        train_loss_epoch = 0
        for batch_X, batch_y, batch_node_ids, batch_vgs, batch_vds in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        ...
        # 每一轮都打印loss
        print(f"Epoch {epoch+1:3d}/{num_epochs}: Train Loss={train_loss_epoch:.6f}, Val Loss={val_loss_epoch:.6f}, LR={current_lr:.6e}")
