...
class TCADTool(BaseTool):
    ...
    async def _run_sdevice(
        ...
        log_path = work_dir / f"sdevice_{type}.log"
        ...
        print(
            f"[SDEVICE][{device_type}][{type}] Live output enabled "
            f"(log: {log_path})",
            flush=True,
        )
        with log_path.open("a", encoding="utf-8") as log_file:
            async for line_bytes in process.stdout:
                ...
                # Keep the complete solver output for later diagnosis.
                log_file.write(line + "\n")
                log_file.flush()

                # Prefix each line because N/P and IV/CV jobs may run concurrently.
                print(
                    f"[SDEVICE][{device_type}][{type}] {line}",
                    flush=True,
                )
        ret = await process.wait()
        del process
